...
def simulate_matchup(teamA, teamB, df, model, feature_names):
    ...
    matchup_df = matchup_df.fillna(0)  # ensure no missing values

    # --- Symmetry correction (averaging both team orientations) ---

    # Original prediction (TeamA vs TeamB)
    prob_original = model.predict_proba(matchup_df)[0][1]  # Team A win probability

    # Swapped features (TeamB vs TeamA)
    ...
    # 🩹 FIX: make sure columns are in the same order as training
    swapped_features = swapped_features[feature_names]
    swapped_features = swapped_features.fillna(0)

    # Predict using the exact same feature order
...
if len(teamA) == 5 and len(teamB) == 5:
	...
	if st.button("🚀 Simulate Matchup", use_container_width=True, type="primary"):
		...
		with viz_col2:
			...
			ax.pie([prob[1], prob[0]], labels=['Team A', 'Team B'], autopct='%1.1f%%', colors=colors, startangle=90, textprops={'fontsize': 12})
		...
		try:
			...
			for col in teamA_stats.columns:
				if col == 'PTS':
					stat_names.append('Points')
				elif col == 'AST':
					stat_names.append('Assists')
				elif col in ['REB', 'TRB', 'Rebounds']:
					stat_names.append('Rebounds')
				elif col == 'STL':
					stat_names.append('Steals')
				elif col == 'BLK':
					stat_names.append('Blocks')
				else:
